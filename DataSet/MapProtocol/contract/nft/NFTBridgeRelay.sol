...
contract NFTBridgeRelay is Role{
    ...
    function _mintFee(uint fromChain,uint toChain) internal {
        (address fromNative, uint fee) = feeNFT.getChainNativeTokenAndFee(fromChain,toChain);
        if (fee > 0 && fromNative != address (0)){
    ...
    function transferInNFT(address _token, address to, uint tokenID, uint fromChain, uint toChain, uint nativeChain,
        ...
        if (chainId != toChain){
            emit mapTransferOutNFT(orderId,_token, tokenID, fromChain, chainId, nativeChain);
            return;
        }
